...
const ARTICLE_STRETCH_LIMIT = 220;
const MIN_AD_HEIGHT = 150;

const AD_COPY = [
  {
    label: "Advertisement",
    title: "Morning Tea At Crown Cafe",
    body: "Fresh buns, strong tea, and a table by the window from 7 a.m."
  },
  {
    label: "Public Notice",
    title: "Book Sale This Saturday",
    body: "The library friends group will sell donated books beside the reading room."
  },
  {
    label: "Advertisement",
    title: "Tailoring Done By Friday",
    body: "Alterations, school uniforms, and repairs at Patel Stitch House."
  },
  {
    label: "Classified",
    title: "Piano Lessons",
    body: "Beginner evening classes now open near the town hall."
  },
  {
    label: "Advertisement",
    title: "Cycle Repairs",
    body: "Brake work, tire tubes, and chain service behind the old market."
  }
];
...
export function splitImages(markdown) {
  const images = [];
...
export function titleFontSize(colSpan, index, continued) {
  if (continued) {
...
export function prepareLayoutItems(articles) {
...
export function widthCandidates(item) {
  if (item.index === 0) {
    return [4];
  }
  ...
  const base = item.kind === "image" ? Math.max(2, item.article.width) : item.article.width;
  const wider = item.article.articleLength > 1100 ? base + 1 : base;
  const candidates = [base, wider, base + 1, base - 1, base + 2, GRID_COLUMNS]
    .map((width) => clamp(width, item.kind === "image" ? 2 : 2, GRID_COLUMNS))
    .filter((width, index, all) => all.indexOf(width) === index);

  return candidates;
}

function measuredHeightFor(measurements, item, width) {
  return measurements[`${item.id}-${width}`] || estimatedHeight(item, width);
}

function placementWidths(item) {
  return widthCandidates(item).filter((width) => {
    if (width === GRID_COLUMNS) {
      return item.index === 0 || item.article.articleLength > 2200;
    }

    if (item.index > 0 && width > item.article.width + 1) {
      return false;
    }
...
function placementCandidates(columnHeights, item, measurements, bodyHeight) {
  const candidates = [];

  placementWidths(item).forEach((width) => {
    const height = Math.ceil(measuredHeightFor(measurements, item, width));
    ...
    for (let x = 0; x <= GRID_COLUMNS - width; x += 1) {
      ...
      const y = Math.max(...columnHeights.slice(x, x + width));

      if (y + height > bodyHeight) {
        continue;
      }

      const nextHeights = [...columnHeights];
      for (let column = x; column < x + width; column += 1) {
        nextHeights[column] = y + height;
      }

      const balance = Math.max(...nextHeights) - Math.min(...nextHeights);
      const rightRailPenalty = x + width === GRID_COLUMNS ? 18 : 0;
      const leftRailPenalty = x === 0 && y > 0 ? 14 : 0;
      const deterministicDrift = ((item.index + 1) * 13 + x * 17 + width * 7) % 29;
      const score =
        y * 0.7 +
        balance * 0.34 +
        Math.abs(width - item.article.width) * 92 +
        width * 9 +
        rightRailPenalty +
        leftRailPenalty +
        deterministicDrift;

      candidates.push({ x, y, w: width, h: height, score });
    }
  });
...
function columnsFor(block) {
  return Array.from({ length: block.w }, (_value, index) => block.x + index);
}

function adFor(pageIndex, adIndex, x, y, w, h) {
  const ad = AD_COPY[(pageIndex * 3 + adIndex + x + w) % AD_COPY.length];

  return {
    kind: "ad",
    id: `ad-${pageIndex}-${adIndex}-${x}-${Math.round(y)}`,
    x,
    y,
    w,
    h,
...
function stretchBottomArticles(blocks, columnHeights, bodyHeight) {
  ...
  for (let index = nextBlocks.length - 1; index >= 0; index -= 1) {
    ...
    const span = columnsFor(block);
    const isBottomBlock = span.every((column) => Math.abs(nextHeights[column] - bottom) < 1);

    if (!isBottomBlock) {
      continue;
    }

    const gap = bodyHeight - bottom;

    if (gap <= 0 || gap > ARTICLE_STRETCH_LIMIT) {
      continue;
    }
    ...
    span.forEach((column) => {
      nextHeights[column] = bodyHeight;
    });
  }

  return { blocks: nextBlocks, columnHeights: nextHeights };
}

function bestAdSpace(columnHeights, bodyHeight, minimumHeight) {
  let best = null;

  for (let x = 0; x < GRID_COLUMNS; x += 1) {
    for (let w = 2; w <= GRID_COLUMNS - x; w += 1) {
      const y = Math.max(...columnHeights.slice(x, x + w));
      const h = bodyHeight - y;
...
function finishMeasuredPage(blocks, columnHeights, bodyHeight, pageIndex, isFinalPage) {
  const stretched = stretchBottomArticles(blocks, columnHeights, bodyHeight);
  const finishedBlocks = [...stretched.blocks];
  const finishedHeights = [...stretched.columnHeights];
  const minimumHeight = isFinalPage ? 96 : MIN_AD_HEIGHT;
  let adIndex = 0;

  while (adIndex < 8) {
    const space = bestAdSpace(finishedHeights, bodyHeight, minimumHeight);

    if (!space) {
      break;
    }

    const adHeight = Math.min(space.h, isFinalPage ? 420 : 360);
    finishedBlocks.push(adFor(pageIndex, adIndex, space.x, space.y, space.w, adHeight));

    for (let column = space.x; column < space.x + space.w; column += 1) {
      finishedHeights[column] = space.y + adHeight;
    }
  ...
  return {
    ...
    columnHeights: finishedHeights
  };
}

export function createMeasuredPages(items, measurements, pageHeight = PAGE_HEIGHT) {
  const bodyHeight = pageHeight - FOLIO_HEIGHT;
  const pages = [];
  let blocks = [];
  let columnHeights = Array(GRID_COLUMNS).fill(0);

  items.forEach((item) => {
    let candidates = placementCandidates(columnHeights, item, measurements, bodyHeight);

    if (candidates.length === 0 && blocks.length > 0) {
      const finished = finishMeasuredPage(blocks, columnHeights, bodyHeight, pages.length, false);
      pages.push({
        blocks: finished.blocks,
        height: Math.max(bodyHeight, Math.max(...finished.columnHeights) + FOLIO_HEIGHT)
      });
      blocks = [];
      columnHeights = Array(GRID_COLUMNS).fill(0);
      candidates = placementCandidates(columnHeights, item, measurements, bodyHeight);
    }

    const oversizedWidth = GRID_COLUMNS;
    const oversizedHeight = Math.ceil(measuredHeightFor(measurements, item, oversizedWidth));
    const placement =
      candidates[0] || {
        x: 0,
        y: 0,
    ...
    blocks.push({
      ...item,
      ...placement,
      bodyColumns: item.kind === "article" ? bodyColumnCount(item.article, placement.w) : 1,
      titleSize:
        item.kind === "article" && item.showTitle
          ? titleFontSize(placement.w, item.index, item.continued)
          : undefined
    });

    for (let column = placement.x; column < placement.x + placement.w; column += 1) {
      columnHeights[column] = placement.y + placement.h;
    }
  });

  if (blocks.length > 0) {
    const finished = finishMeasuredPage(blocks, columnHeights, bodyHeight, pages.length, true);
    pages.push({
      blocks: finished.blocks,
      height: Math.max(bodyHeight, Math.max(...finished.columnHeights) + FOLIO_HEIGHT)
    });
